...
const HireMessage = () => {
	...
	return (
		<section className={`${styles.hire_message} container`}>
				...
				<p className={styles.title2}>

					{
						processBoldText(content.title2).map((text, index) => {
							...
							return <span key={index} className={styles.bold_title}>{text.content} </span>
			...
			{
				isDesktop ? <img src={banner_desktop} alt="" /> : <img src={banner_mobile} alt="" />
			}

			{
				isOpenModal && <HireMessageModal closeModal={() => { setIsOpenModal(false) }} />
			}
